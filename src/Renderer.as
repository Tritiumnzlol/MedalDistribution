...
)
{
    ...
    return Clamp01(
        float(state.PBBoundary)
        /
        float(
            state.Distribution.PlayerCount
        )
    );
}
...
)
{
    return
        "None\n"
        + FormatMedalPercent(
            stats.NoMedalPercent(),
            stats.BronzeCoarse
                || stats.PlayerCountCoarse,
            stats.NoMedalUnresolved()
        );
}
...
void DrawHorizontalTripleFuzzyBoundary(
    string id,
    vec2 barPos,
    float boundaryX,
    float barLength,
...
void DrawVerticalFuzzyBoundary(
    string id,
    vec2 barPos,
    float boundaryY,
    float barWidth,
...
void DrawVerticalTripleFuzzyBoundary(
    string id,
    vec2 barPos,
    float boundaryY,
    float barWidth,
...
int AssignLabelLanes(
    array<float>@ centers,
    array<int>@ lanes
)
{
    array<float> lastCenter =
    {
        -100000.0f,
        -100000.0f,
        -100000.0f
...
)
{
    ...
    if (goldUnresolved)
    {
        DrawHorizontalTripleFuzzyBoundary(
            "GoldUnresolvedFuzzy",
            barPos,
            authorX,
            length,
    ...
    {
        UI::Text(
            "~ approximate   ? unresolved at leaderboard resolution"
        );
    }
...
)
{
    ...
    {
        DrawVerticalFuzzyBoundary(
            "AuthorGoldVerticalFuzzy",
            barPos,
            authorY,
            width,
            length,
    ...
    if (goldUnresolved)
    {
        DrawVerticalTripleFuzzyBoundary(
            "GoldUnresolvedVerticalFuzzy",
            barPos,
            authorY,
            width,
            length,
    ...
    // ---------------------------------------------------------
    // Labels - now actually beside the vertical bar.
    // ---------------------------------------------------------
...
void RenderMedalDistribution()
{
    ...
    bool visible =
        UI::Begin(
            "Medal Distribution###MedalDistributionOverlay",
            flags
        );
